...
[*Code template*](code/README.md) - Starting code framework for the (insert your exact problem here.)
...
* Because we are comparing group averages,we can use an ANOVA to compare capacity and transit time accross groups in our simulation
* A clusted bar chart will be used to visual group differences between 3 set speeds of the escalator
* We can embedd the data from our clustered bar graph into an infograph thats looks like a series of escalators. The size of the escalator will correspond to our output variables (capacity and average transit time).
...
Looking at the differences (above) of random samples from three different tpyes of distribtuions, its clear that as our sample size gets larger the sample gets closer to the population distribution. With small sample sizes (n=10), it's hard to notice much of a difference between samples of different distributions. 

![Quasi-Random Generated Distributions](images/quasi_dist.png)

When using a Quasi-Random generator to create distribtutions, the generated data is much more closely fit to the sampled distribution at lower N's. Samples generated this way probably have considerably less variability. 
